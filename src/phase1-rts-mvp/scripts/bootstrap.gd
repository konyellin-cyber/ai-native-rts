...
# ─── Public API（供 CommandRouter 等外部调用，不依赖内部字段名）────────
func get_config() -> Dictionary:
	return config

func get_units_for_debug() -> Array:
	## 返回 _world.units（所有在场单位列表），供 command_router unit_info 使用
	if _world == null:
		return []
	return _world.units if _world.get("units") != null else []

func get_red_alive() -> int:
	## 返回红队存活数，供 command_router unit_info 使用
	if _lifecycle == null:
		return 0
	return _lifecycle.red_alive if _lifecycle.get("red_alive") != null else 0

func get_blue_alive() -> int:
	## 返回蓝队存活数，供 command_router unit_info 使用
	if _lifecycle == null:
		return 0
	return _lifecycle.blue_alive if _lifecycle.get("blue_alive") != null else 0
